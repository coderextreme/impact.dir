...
public class Cell extends Component implements MouseMotionListener, MouseListener, KeyListener, Runnable {
	...
	public static void main(String args[]) {
		...
		if (personalities_str.equals("cell")) {
			...
			((BufferP)Common.personalities[1][0]).addToInBuffer('1');
/*
		} else if (new File(personalities_str).exists()) {
			try {
				FileInputStream fis = new FileInputStream(personalities_str); 
...
*/
			...
			try {
				System.err.println("Opening "+personalities_str+" from jar");
				InputStream is = jf.getClass().getClassLoader().getResource(personalities_str).openStream();
		...
		try {
			...
			for (int y = 0; y < Common.MMAXY; y++) {
				for (int x = 0; x < Common.MMAXX; x++) {
					...
					Common.modulePersonalities[x][y].lOutput = br.readLine();
					Common.modulePersonalities[x][y].rOutput = br.readLine();
					Common.modulePersonalities[x][y].tOutput = br.readLine();
					Common.modulePersonalities[x][y].bOutput = br.readLine();
