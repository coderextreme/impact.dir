		...
		if (bgather) {
			...
			if (!bflag) {
				...
				if (rnfull) {
					...
					b <<= 1;
					try {
						if (rightNeighbor.consumeLeftOutput().equals("1")) {
							b |= 1L;
						} else {
							b &= ~1L;
						}
				...
				if (rnfull) {
					...
					try {
						Common.changed = true;
						bend =  rightNeighbor.consumeLeftOutput().equals("1");
		...
		if (agather) {
			...
			if (!aflag) {
				...
				if (tnfull) {
					...
					try {
						if (topNeighbor.consumeBottomOutput().equals("1")) {
							a |= 1L;
						} else {
							a &= ~1L;
						}
				...
				if (tnfull) {
					...
					try {
						Common.changed = true;
						aend = topNeighbor.consumeBottomOutput().equals("1");
			...
			Common.changed = true;
			if (a < b) {
				Common.personalities[x][y].setLeftOutput("0");
				a <<= 1;
				leftshifts++;
				...
				Common.personalities[x][y].setLeftOutput("1");
				a -= b;
				a <<= 1;
				leftshifts++;
				System.err.println("leftshifts++ a > b, a = "+a);
			...
			if (!flag) {
				...
				if (leftshifts > 0) {
					...
					if (masked != 0) {
						sentaone = true;
						Common.personalities[x][y].setBottomOutput("1");
					} else if (sentaone) {
						Common.personalities[x][y].setBottomOutput("0");
					}
					flag = true;
				}
			} else {
