...
public class Picking
{
  ...
  Picking()
  {
    ...
    drawable = new GLCanvas(capabilities);
    ...
    public void init(GLAutoDrawable drawable) 
    {
      ...
      for (int i = 0; i < paths.length; i++) {
	    System.err.println("initing paths "+ov.x+" "+nv.x);
            paths[i] = nv;
    	    color[i] = new float[] { random.nextFloat(), random.nextFloat(), random.nextFloat(), 1.0f };
	    ov = nv;
   	    nv = new Vect();
	    nv.x = random.nextFloat()*10-5;
	    nv.y = random.nextFloat()*10-5;
...
/*
  public void reshape(GLJPanel drawable, int x, int y, int width, int height) {
...
*/
  ...
  public void drawScene(GL2 gl) {
    ...
    for (int i = 0; i < paths.length; i++) {
	    gl.glMaterialfv(GL2.GL_FRONT, GL2.GL_AMBIENT_AND_DIFFUSE, color[i], 0);
...
*/
    ...
    public void displayChanged(GLJPanel drawable, boolean modeChanged, boolean deviceChanged) {}
