...
public class Impact3D implements GLEventListener, MouseListener, MouseMotionListener {
  ...
  public void init(GLAutoDrawable drawable) {
    // Use debug pipeline
    // drawable.setGL(new DebugGL(gl = drawable.getGL()));
    gl = (GL2)drawable.getGL();

    ((ComponentEvents)drawable).addKeyListener(new KeyAdapter() {
        public void keyPressed(KeyEvent e) {
		GLCanvas drawable = (GLCanvas)e.getSource();
		key = e.getKeyCode();
		float x = 0f;
		float y = 0f;
		float z = 0f;
...
*/
    gl.glLoadIdentity();
    ((GL2ES1)gl).glFrustum(-1.0f, 1.0f, -asp, asp, 5.0f, 600.0f);
  ...
  public void drawScene(GLAutoDrawable drawable, float view_rotx, float view_roty, float view_rotz, float view_posx, float view_posy, float view_posz) {
    gl.glMatrixMode(GL2.GL_MODELVIEW);
    gl.glInitNames();
    ...
    public void display(GLAutoDrawable drawable) 
    {
      ...
      switch(cmd)
        {
        case UPDATE:
          drawScene(drawable, vrx, vry, vrz, vpx, vpy, vpz);
	  cmd = NOTHING;
          break;
        case SELECT:
          int buffsize = 512;
          ...
          ((GL2ES1)gl).glFrustum(-1.0f, 1.0f, -a, a, 5.0f, 600.0f);
          drawScene(drawable, vrx, vry, vrz, vpx, vpy, vpz);
          gl.glMatrixMode(GL2.GL_PROJECTION);
          gl.glPopMatrix();
          // gl.glFlush();
...
*/
  ...
  public void displayChanged(GLCanvas drawable, boolean modeChanged, boolean deviceChanged) {}
